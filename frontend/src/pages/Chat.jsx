...
export default function Chat() {
  ...
  // Open a single WS connection for the page
  useEffect(() => {
    ...
    ws.onmessage = (evt) => {
      ...
      if (data.type === 'final' || data.type === 'cached') {
        setMessages((m) => {
          const next = [...m]
          // Replace the trailing assistant placeholder
  ...
  const send = useCallback(() => {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      ...
      api.query({ query: q, session_id: sessionId })
        ...
        .catch((e) => {
          setMessages((m) => [...m, { id: uid(), role: 'assistant', content: `Error: ${e.message}` }])
        })
  ...
  const onEscalate = (msg) => {
    api.feedback({
      session_id: sessionId, message_id: msg.id, rating: 'down',
      note: 'ESCALATE_TO_HUMAN',
    }).catch(() => {})
    setMessages((m) => [...m, {
